...
$type=$_REQUEST['types'];
  ...
  if(($action_val=="forget_p") and ($type=='1'))
 {
	...
	$chkqry= "select ulog.usrlog_password, ulog.usrlog_username from tbl_user_login ulog,tbl_user_profile up where  up.usrpro_email = '".$uemail."' and up.usrlog_id = ulog.usrlog_id ";
	$selectAffectedRows = $userslog_obj->selectAffectedRows($chkqry);	
	if($selectAffectedRows)
		{
		...
		$username_val = $chk_pass_access[0]['usrlog_username'];	
				...
				$fp_email_tmpl =str_replace("%userpwd%", "$pass_val", $fp_email_tmpl);
...
elseif($action_val=="dropstates")
 {
	...
	$sele_city = '<select id="city_drop" name ="city_id" class="inputval"><option value="0">Select City</option>';
