...
&nbsp;
<div id="content" class="index home" data-pjax-container="" style="opacity: 1; display: block; width: 953px;">   
		<div id="wedding-party" class="clearfix" style="width: 100% !important;">
		{if $glb_pageheading neq ''}<h1 style="text-align: center;"><span id='pageheddings'>{$glb_pageheading}</span></h1>{else}&nbsp;{/if}
				...
				{if $glb_home_img_status eq 1} <span style="padding-top:20px; width: 350px; {if $glb_theme_owner_id eq '6442'} display: none; {/if}" class="col2_2"> {else} <div style="text-align: center;" > {/if}
...
<!-- sara smith -->
				{if $glb_home_img_status eq 1}
				<span style=" {if $glb_theme_owner_id eq '6442'} width: 868px; {else} width: 500px; {/if} padding-top:20px;" class="col2_2">
				{/if}
