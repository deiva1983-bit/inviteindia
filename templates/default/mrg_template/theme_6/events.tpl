...
{/literal}
<!-- content --><h2 style="text-align: center;"><span id='pageheddings'>{$tpl_event_title}</span></h2>
		<section id="content">
			...
			{if $smt_rec_title neq ''}<h2 id='subhead'><span>{$smt_rec_title}</span></h2>
			{else}<h2 id='subhead'><span>Reception:</span></h2>
			{/if}
			...
			{if $smt_reception_date neq ''} <p>{$smt_reception_date}</p> {/if}
			{$smt_reception_location}
			</td><td style='width: 30%; vertical-align: top; padding-right: 120px;'>
			...
			{if $smt_wed_title neq ''}
				<h2 id='subhead'><span>{$smt_wed_title}</span></h2>
			{else}
				<h2 id='subhead'><span>Marriage:</span></h2>
			{/if}
