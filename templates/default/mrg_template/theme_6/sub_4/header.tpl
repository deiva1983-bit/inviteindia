...
<!-- header  -->
		<header>
			<nav>
				<ul id="menu">
						<li {if $c_page_status eq ''} id="menu_active" {/if}><a href="{$glb_page_url}" title="Home">{$glb_wed_card_cover}</a></li>
			...
			{if $c_page_status eq 'h' || $c_page_status eq ''}{if $glb_pageheading neq ''  && $page_ownpage eq ''} <h2 id="pageheddings" style='text-align: center;'>{$glb_pageheading}</h2>{/if}{/if}
		</header>
<!-- / header -->
