...
<div id="left-content" style="margin-left:40px">
	<p>{if $smt_reception_status neq 0}
		<input type='hidden' id='rec_status' value='1' />
		{if $smt_rec_title neq ''}<h2 id="subhead">{$smt_rec_title}</h2>
		{else}<h2 id="subhead">Reception</h2>
		{/if}
	...
	{if $smt_wed_title neq ''}<h2 id="subhead">{$smt_wed_title}</h2>
	{else}<h2 id="subhead">Marriage</h2>
	{/if}
