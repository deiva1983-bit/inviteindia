...
if($current_action=="select") {
    ...
    if($user_allowed){
        ...
        $smarty->assign('errors', "Your invitation trial period has expired, Since you're not selected any package you can't able to access paid themes." );
    }
...
// Classic Themes - But we dont have Own BG image options
    $chkqry= "SELECT mrg_theme_demo_img demo_img, mrg_theme_name theme_name, mrg_theme_url theme_url, mrg_theme_auto_id theme_id, theme_demo_url demo_url, mrg_theme_bg_image bg_img_sts FROM mrg_mas_theme where mrg_theme_status ='1' and mrg_theme_catid = '1' and theme_type = '$invite_type' order by mrg_theme_bg_image desc";
    $selectalbum_access = $userslog_obj->selectVal($chkqry);
    $total_records = count($selectalbum_access);
    if($total_records){
        $classic_commdetails="";
        ...
        foreach($selectalbum_access as $key=>$field) {
            ...
            $themeid =trim($field['theme_id']);
            $demourl =trim($field['demo_url']);
			$bg_img_sts =trim($field['bg_img_sts']);
			$submit_urls = "$submit_page&theme_id=$themeid&might=net";
            $srcid = 'theme_'.$themeid;
            ...
            $classic_commdetails .= "<div class='col-md-4 agileinfo_new_products_grid agileinfo_new_products_grid_mobiles border_bott'>
                                        <div class='agile_ecommerce_tab_left mobiles_grid'>
											...
											$bg_img_icon = '';
											if ($bg_img_sts == 2)
												$bg_img_icon = "<p><i class='fa fa-picture-o' aria-hidden='true'></i></p>";
                                            $classic_commdetails .= "<h3 class='w3layouts_head'>$themename</h3>";
											if($show_select_link && !$blocktheme){
                                            ...
                                            $classic_commdetails .= "<div class='mobiles_grid_pos'>$bg_img_icon<p><i class='fa fa-inr' aria-hidden='true'></i></p>
...
$canurl = $ssl_path.'www.inviteindia.com/invitation-templates.php';
