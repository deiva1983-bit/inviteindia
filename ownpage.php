...
$img_align= trim($_REQUEST['img_align']);
...
if($page_allowed && $listid != ''){
	...
	$radio_highlight = 0; $title_highlight_p1 = ''; $title_highlight_p2 = ''; $title_highlight_p3 = '';
	++$rowcount;
	//***************************************************//
	// Parah 1 Operations
	// Image Upload
		$add_image_p1 = trim($_REQUEST['save_parah_first']);
		...
		$infos_1 = (strlen ($infos_val_1) < 100 ) ? '' : $infos_val_1 ;
		$validateevents = ( $infos_1 != '') ? 1 : 0 ;
		if (!$validateevents){
			$errors .= '<li> Please enter your text. Your text should be more then 100 character.</li>';
			$msg_sts = 1;
		}
		$errors .= '</ul>';
		$smarty->assign('errors', $errors);
		$radio_highlight = 1;
		$title_highlight_p1 = $add_title_p1;
		$highlight_txtarea_1 = $infos_val_1;
		
		//$sett_val = perform_imgupload($wedid, $listid, $_FILES['uploaded_image_6853305'], $user_log_id, 1);
			if(!$msg_sts){
			$radio_highlight = 0;
			$title_highlight_p1 = ''; $highlight_txtarea_1 = '';
			$image_sett = up_setting($wedid, $rowcount, $infos_val_1, $_FILES['uploaded_image_'.$rowcount], $add_title_p1, $img_align, $listid);
			$show_sts=1;
	...
	if($add_image_p2 == 'Save') {
		...
		$smarty->assign('errors', $errors);
		$radio_highlight = 2;
		$title_highlight_p2 = $add_title_p1;
		$highlight_txtarea_2 = $infos_val_1;
		//function up_setting($wedid, $parahid, $parah_texts, $files, $title_val, $parah_align, $listid) {
			if(!$msg_sts){
			$radio_highlight = 0;
			$title_highlight_p2 = ''; $highlight_txtarea_2='';
			$image_sett = up_setting($wedid, $rowcount, $infos_val_1, $_FILES['uploaded_rightimage_'.$rowcount], $add_title_p1, $img_align, $listid);
			$show_sts=1;
			$succ_msg = 'Your parah settings successfully updated.';
			}
		}
	...
	if($add_image_p3 == 'Save') {
		...
		$smarty->assign('errors', $errors);
		$radio_highlight = 3;
		$title_highlight_p3 = $add_title_p1;
		$highlight_txtarea_3 = $infos_val_1;
		//function up_setting($wedid, $parahid, $parah_texts, $files, $title_val, $parah_align, $listid) {
			$title_highlight_p3 = $add_title_p1;
			if(!$msg_sts){
			$radio_highlight = 0;
			$title_highlight_p3 = ''; $highlight_txtarea_3='';
	...
	$pcnt++;
	$smarty->assign('glb_rowcount', $pcnt);
	//***************************************************//
	// Parah 1 status
	$p1 = 0;
...
// PARAH STATUS
$smarty->assign('glb_title_highlight_p1', $title_highlight_p1);
$smarty->assign('glb_title_highlight_p2', $title_highlight_p2);
$smarty->assign('glb_title_highlight_p3', $title_highlight_p3);

$smarty->assign('glb_highlight_txtarea_1', $highlight_txtarea_1);
$smarty->assign('glb_highlight_txtarea_2', $highlight_txtarea_2);
$smarty->assign('glb_highlight_txtarea_3', $highlight_txtarea_3);


$smarty->assign('glb_radio_highlight', $radio_highlight);
