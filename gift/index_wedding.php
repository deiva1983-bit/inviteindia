...
//http://affiliate.flipkart.com/install-app?affid=deivainvi
// access-token - a670edcd481f40a2b521062bc61b3362
...
$topoffers_url = 'https://affiliate-api.flipkart.net/affiliate/1.0/search.json?query=wedding+gifts+couples&resultCount=20';
...
if($url){
	...
	if(count($products) > 0){
		foreach ($products as $product) {
			...
			if($ownstyle) {
			echo '<a target="_blank" href="'.$productUrl.'"><img src="'.$productImage.'" style="max-width: 200px;" /><br>'.$title."</a><br>Rs. ".$sellingPrice;
			} else {
...
if($offer){
	...
	if(count($products) > 0){
		foreach ($products as $product) {
			...
			$productDescription = '';
			if($ownstyle) {
			echo '<a target="_blank" href="'.$productUrl.'"><img src="'.$productImage.'" style="max-width: 200px;" /><br>'.$title."</a><br>".$productDescription."<br>Rs. ".$sellingPrice;
			} else {
			echo '<a target="_blank" href="'.$productUrl.'"><img src="'.$productImage.'" /><br>'.$title."</a><br>".$productDescription."<br>Rs. ".$sellingPrice;
			}
...
/*
		...
		if(count($list) > 0){
			foreach ($list as $item) {
				...
				$description = $item['productBaseInfo']['productAttributes']['productDescription'];
				//$description = $item['description'];
				$url = $item['url'];
				$imageUrl = $item['imageUrls'][0]['url'];
				$availability = $item['availability'];
