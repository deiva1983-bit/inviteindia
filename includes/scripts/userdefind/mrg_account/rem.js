$(document).ready(function()
{
    ...
    $(function() {
        ...
        var rem_mobno = $("#rem_mobno"),rem_name = $("#rem_name"),rem_email = $("#rem_email"),rem_date = $("#marriage_date"),wed_a_id = $("#wed_auto_id"),wed_valid_date = $("#wed_validation_date"),wed_curr_date = $("#wed_curr_date"),allFields_L = $([]).add(rem_name).add(rem_mobno).add(rem_email).add(rem_date).add(wed_a_id),tips1 = $(".validateTipsRem"); //alert (rem_mobno.val());
        ...
        var overlayheight =$("#overlay_height").val();
        ...
        //}
        $("#dialog-form").dialog({ 
            autoOpen: false,height: overlayheight,width: 430,modal: true,
            buttons: {
                ...
                var bValid = true;
                    if(rem_date.val() != ''){
                        ...
                        //if (startDate > todaydate && startDate <= endDate) {
                            ...
                            updateTips("Please select a valid date. Your reminder date should not exist wedding date.",tips1);
                            bValid=false;
                            }
                    ...
                    if (bValid){
                        ...
                        $.post("ajaxfiles/setrem.php",{rem_nam:rem_name.val(), remail:rem_email.val(),rmobno:rem_mobno.val(),rdate:rem_date.val(),wed_au_id:wed_a_id.val(),chk_action:'setrem'},function(result){
                            ...
                            updateTips_Success("Congrats, Your reminder successfully updated.",tips1);
        ...
        $('#activate_now').click(function(){
            var rem_mobno = $("#rem_mobno"), rem_act_code = $("#rem_activate_code"), allFields_L = $([]).add(rem_mobno).add(rem_act_code), tips1 = $(".validateTipsRem");
            var bValid = true;
            bValid = bValid && checkEmpty(rem_mobno,"Mobile number",tips1);
            bValid = bValid && checkNumberOnly(rem_mobno,"Enter Valid Mobile number",tips1);
            bValid = bValid && checkLength(rem_mobno,"Mobile must 10 digits",tips1,10);
                ...
                $.post("ajaxfiles/setrem.php",{act_code:rem_act_code.val(),rmobno:rem_mobno.val(),chk_action:'doActivate'},function(result){
                    if(result == "wrong"){
                        ...
                        rem_act_code.focus();
                            ...
                            updateTips_Success("Your number activated successfully. Please click the Set reminder button now!",tips1);
                            rem_act_code.val("");
        ...
        $('#call_remaninder').click(function() {
        $('#dialog-form').dialog('open');
        tips = $(".validateTips");
        tips.text("All form fields are required.").addClass('ui-state-highlight');
        });
    });
}); 
