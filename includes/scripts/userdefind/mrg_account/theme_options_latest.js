         ...
         if (createWedForm())
            ...
            bValid = bValid && checkLengthminmax(brides_name,"bride's name",3,30,tips);
                if (bValid){
					...
					$("#validateTips_div").show("fast");
                    return false;
				}
                 
            }
    });
 $("#butt_general_info").click(function(){
			         if (createWedForm())
            return false;
         else {
            var home_imagessts = $('input[name=home_images_sts]:checked', '#wed_account_create').val();
            var wed_url = $("#wed_url"),male_img = $("#uploaded_homeimage_male"),female_img = $("#uploaded_homeimage_fmale"),weddate = $("#weddate"),allFields = $([]).add(wed_url).add(male_img).add(female_img),tips = $("#validateTips");
            var bValid = true;
            allFields.removeClass('ui-state-error');
            bValid = bValid && checkEmpty(wed_url,"Website URL",tips);
...
$("#frm_wedding_info").click(function(){
	return true;
        ...
        $("#frm_reception_info").click(function(){
			return true;
            //var wed_date = $("#marriage_date"), rec_status = $("#reception_status"), rec_date = $("#reception_date"),
            var rec_status = $("#reception_status"), rec_date = $("#reception_date"),
            //allFields = $([]).add(wed_date).add(rec_status).add(rec_date),
